...
impl TankCalcActor {
    async fn process_tank(&self, tank_id: &Uuid, state: &mut TankCalcState) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut last_ts = if *current_index > 0 { time_series[*current_index - 1].ts } else { 0 };
    ...
    fn calculate_base_vars(&self, meta: &Meta, entry: &TimeSeriesEntry, now: DateTime<Utc>) -> BaseVarsWithDate {
        ...
        let product_avg_temp = ((entry.t0 + entry.t1 + entry.t2 + entry.t3 + entry.t4 + entry.t5 + entry.t6 + entry.t7 + entry.t8 + entry.t9) / 10.0).round();
    ...
    fn calculate_ext_vars(&self, meta: &Meta, entry: &TimeSeriesEntry, now: DateTime<Utc>) -> ExtVarsWithDate {
        // РАСЧЕТ EXT_VARS
        ...
        let temperatures = vec![
            Temperature { value: entry.t0, level: (entry.h_measured * 0.0 / 9.0).round(), name: "T0".to_string() },
            Temperature { value: entry.t1, level: (entry.h_measured * 1.0 / 9.0).round(), name: "T1".to_string() },
            Temperature { value: entry.t2, level: (entry.h_measured * 2.0 / 9.0).round(), name: "T2".to_string() },
            Temperature { value: entry.t3, level: (entry.h_measured * 3.0 / 9.0).round(), name: "T3".to_string() },
            Temperature { value: entry.t4, level: (entry.h_measured * 4.0 / 9.0).round(), name: "T4".to_string() },
            Temperature { value: entry.t5, level: (entry.h_measured * 5.0 / 9.0).round(), name: "T5".to_string() },
            Temperature { value: entry.t6, level: (entry.h_measured * 6.0 / 9.0).round(), name: "T6".to_string() },
            Temperature { value: entry.t7, level: (entry.h_measured * 7.0 / 9.0).round(), name: "T7".to_string() },
            Temperature { value: entry.t8, level: (entry.h_measured * 8.0 / 9.0).round(), name: "T8".to_string() },
            Temperature { value: entry.t9, level: (entry.h_measured * 9.0 / 9.0).round(), name: "T9".to_string() },
        ];

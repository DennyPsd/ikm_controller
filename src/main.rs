...
fn main() -> Result<(), ModuleError> {
    Module::init(IPCRole::Router).map(|module| {
        module.run(async |_cfg, module, ipc_router, _| {
            ...
            // initial_devices пустой
            let initial_devices: Vec<FacilityDevice> = Vec::new();

            // ----- ModbusFabric ---------
            let (modbus_fabric, _modbus_fabric_handle) = module
                .spawn_linked(Some("ModbusFabric".into()), ModbusFabricActor::new(), (initial_devices, modbus_settings.clone()))
                .await
            ...
            ipc_router
                .subscribe(ipc_handler, |msg| Some(IpcHandlerMsg::Ipc(msg)))
